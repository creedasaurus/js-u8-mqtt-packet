...
export class mqtt_reader_v4 {
  ...
  utf8() {
    return new TextDecoder("utf-8").decode(this.bin());
  }
  pair() {
    return [this.utf8(), this.utf8()];
  }

  flags(FlagsType) {
    return new FlagsType(this.buf[this.step(1)]);
  }

  reason(pkt_kind) {
    let v = this.buf[this.step(1)];
    if (null != v) return U8_Reason.of(v, pkt_kind, this._reasons_by);
  }
...
export class mqtt_reader_v5 extends mqtt_reader_v4 {
  props() {
    ...
    let res = {},
      fork = this.of(buf.subarray(vi, step.k | 0));
    while (fork.has_more()) {
      let pt = mqtt_props.get(fork.u8()),
        value = fork[pt.type]();
      res[pt.name] = !pt.op ? value : fork[pt.op](res[pt.name], value);
    }
    return res;
  }
...
export function mqtt_reader_info(mqtt_reader, ...info_fn_list) {
  mqtt_reader = class extends mqtt_reader {
    static reasons(pkt_type, ...reason_entries) {
      ...
      for (let [u8, reason] of reason_entries) lut.set(u8, reason);

      return this;
    }
  };

  for (let fn_info of info_fn_list) fn_info(mqtt_reader);
