...
export class mqtt_writer_v4 {
  ...
  vint(v) {
    this.push(encode_varint(v));
  }

  bin(u8_buf) {
    if (!u8_buf) return this.u16(0);
    if ("string" === typeof u8_buf) return this.utf8(u8_buf);

    if (u8_buf.length !== u8_buf.byteLength) u8_buf = new Uint8Array(u8_buf);
  ...
  flags(v, enc_flags, b0 = 0) {
    if (undefined !== v && isNaN(+v)) v = enc_flags(v, 0);
  ...
  flush(buf) {
    if (null != buf)
      this.push(
        "string" === typeof buf ? new TextEncoder("utf-8").encode(buf) : buf,
      );
...
export class mqtt_writer_v5 extends mqtt_writer_v4 {
  props(props) {
    ...
    if (0 === props.length) return this.u8(0);
