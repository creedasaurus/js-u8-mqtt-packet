...
function test_mqtt_suite_mqtt_ctx(mqtt_ctx_v4, suite_prefix) {
  ...
  describe(`${suite_prefix}: small pub/sub capture`, () => {
    ...
    it("connack", () => {
      const { type, reason, flags, ...tip } = _decode_one_hex("20020000");

      expect(type).to.equal("connack");

      expect(tip).to.deep.equal({ b0: 0x20 });
    ...
    it("puback c2s with qos:1 (ejabberd)", () => {
      const { type, ...tip } = _decode_one_hex("40029b36");
    ...
    it("subscribe", () => {
      ...
      for (const each of log) {
        const { type, hdr, pkt_id, topics, ...tip } = _decode_one_hex(each);
    ...
    it("suback", () => {
      ...
      const packet_id_list = [];
      for (const each of log) {
        const { type, pkt_id, answers, ...tip } = _decode_one_hex(each);
