...
describe("mqtt v5: small pub/sub capture", () => {
  it("pingreq", () => {
    ...
    expect(tip).to.deep.equal({ b0: 0xc0 });
  });
  ...
  it("pingresp", () => {
    ...
    expect(tip).to.deep.equal({ b0: 0xd0 });
  });
  ...
  it("disconnect", () => {
    ...
    expect(tip).to.deep.equal({ b0: 0xe0 });
  });
  ...
  it("connack", () => {
    const { type, reason, flags, ...tip } = _decode_one_hex(
      "202d00002a22000a12002435373846413945372d433438322d394444312d384632332d433838413042383245343046",
    );
  ...
  it("connack with ident (mosquitto)", () => {
    const { type, reason, flags, ...tip } = _decode_one_hex("200600000322000a");
  ...
  it("connack with ident (ejabberd)", () => {
    const { type, reason, flags, ...tip } = _decode_one_hex(
      "201000000d2200642a00110000000013003c",
    );
  ...
  it("publish c2s", () => {
    const { type, payload, ...tip } = _decode_one_hex(
      "3013000a746573742f746f706963006a656c6b6b6b",
    );
  ...
  it("publish c2s with qos:1", () => {
    const { type, payload, ...tip } = _decode_one_hex(
      "3214000a746573742f746f7069630001006a656c6c6f",
    );
  ...
  it("puback s2c with qos:1", () => {
    const { type, ...tip } = _decode_one_hex("40020001");
  ...
  it("puback c2s with qos:1 (ejabberd)", () => {
    const { type, ...tip } = _decode_one_hex("40022fa3");
  ...
  it("connect", () => {
    ...
    for (const each of log) {
      const { _base_, type, client_id, flags, ...tip } = _decode_one_hex(each);
  ...
  it("subscribe qos: 0", () => {
    const { type, hdr, topics, ...tip } = _decode_one_hex(
      "8210000100000a746573742f746f70696300",
    );
    ...
    ).to.deep.equal([{ topic: "test/topic", opts: 0 }]);
  });

  it("subscribe qos: 1", () => {
    const { type, hdr, topics, ...tip } = _decode_one_hex(
      "8210000100000a746573742f746f70696301",
    );
    ...
    expect(
      topics.map(({ topic, opts }) => ({ topic, opts: +opts })),
    ).to.deep.equal([{ topic: "test/topic", opts: 1 }]);
  });
  ...
  it("publish s2c (mosquitto)", () => {
    const { type, hdr, payload, ...tip } = _decode_one_hex(
      "3012000a746573742f746f706963006a656c6c6f",
    );
  ...
  it("publish s2c (jabberd)", () => {
    const { type, hdr, payload, ...tip } = _decode_one_hex(
      "3214000a746573742f746f706963cea0006a656c6c6f",
    );
