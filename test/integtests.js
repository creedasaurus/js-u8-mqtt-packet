...
export function integ_suite(ms_delay, _new_mqtt) {
  const delay = (n) => new Promise((y) => setTimeout(y, n * ms_delay));
  ...
  const init_mqtt = () =>
    new Promise((done) => {
      ...
      async function on_mqtt(pkt_list, ctx) {
        for (let pkt of pkt_list) integ_log.push(pkt.type);
  ...
  it("connect / disconnect", async () => {
    ...
    assert.equal([...tags].sort().join(" "), "connack");
  });
  ...
  it("subscribe / publish", async () => {
    ...
    let tags = new Set(my_mqtt[sym_integ_log]);
    assert.equal([...tags].sort().join(" "), "connack publish suback");
  });
}
