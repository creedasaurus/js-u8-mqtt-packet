...
stat_outputs().then(show_stat_table);

async function stat_outputs() {
  let dir = await readdir(local_path);
  dir = dir.filter((e) => e.includes(".js")).sort();
  ...
  for (let e of dir) {
    ...
    kind = kind ? kind.replace(/^\./, "") : name === name0 ? "raw" : "min";
...
async function show_stat_table(by_name) {
  let out = out_doc_path ? createWriteStream(out_doc_path) : process.stdout;
